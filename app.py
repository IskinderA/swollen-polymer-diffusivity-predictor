...
def show_prediction_summary(pred: PredictionResult, inputs: SimplePredictorInputs) -> None:
    ...
    with st.expander("How to interpret sampling and model agreement"):
        st.write(
            "The prediction summary reports percentiles calculated directly from the "
            "underlying QRF tree predictions and MLP ensemble predictions. The downloaded "
            "Monte Carlo samples are generated by sampling with replacement from those "
            "empirical distributions. Therefore, sample-summary percentiles may differ "
            "slightly from the displayed prediction-summary percentiles, especially for "
            "smaller requested sample sizes."
        )

        st.write(
            "Model agreement is based on the absolute difference between the QRF and MLP "
            "median predictions in log10(D) units:"
        )

        st.markdown(
            "- **Excellent/small disagreement:** < 0.25 log units\n"
            "- **Moderate disagreement:** 0.25–0.75 log units\n"
            "- **Large disagreement:** > 0.75 log units"
        )
