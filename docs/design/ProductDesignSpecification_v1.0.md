...
The predictor shall allow users to generate empirical prediction samples for downstream uncertainty propagation, visualization, and Monte Carlo analyses.

Users shall specify the desired number of generated samples prior to prediction.

The requested sample size may exceed the number of underlying model components because sampling is performed with replacement.

### QRF Sampling

The QRF prediction distribution is derived from prediction variability across the **300 decision trees** comprising the published random forest model.

Empirical prediction samples are generated by bootstrap resampling from the tree-level prediction distribution.

### MLP Ensemble Sampling

The MLP prediction distribution is derived from prediction variability across an ensemble of **20 independently trained neural networks** initialized using different random seeds.

Empirical prediction samples are generated by bootstrap resampling from the ensemble prediction distribution.

### User Guidance

The predictor shall clearly communicate that the QRF and MLP prediction distributions arise from fundamentally different sources of model variability.

Although both approaches generate empirical prediction distributions, they should not be interpreted as calibrated probabilistic confidence intervals.

Instead, the distributions provide complementary representations of prediction variability that may be used for exploratory analysis, uncertainty propagation, and comparison between the independent model classes.

The prediction sampling controls shall be accompanied by concise explanatory text describing the origin of each distribution and the role of bootstrap resampling.
